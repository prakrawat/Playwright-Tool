...
const configuration = ({
  ...
  use: {
    ...
    // trace: 'on', 
    // Set trace collection to 'on', which means that Playwright will capture detailed information about the test execution, including network requests, DOM snapshots, and other relevant data. This can be helpful for debugging and understanding test failures.
    // Set trace retention to 'retain-on-failure', which means that Playwright will keep the trace information only for tests that fail. This helps to save storage space by not retaining traces for successful tests, while still providing valuable debugging information for failed tests.
    trace: 'on',
